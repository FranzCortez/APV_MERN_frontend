...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if( [ email, password ].includes('') ){
            setAlerta({
                msg: 'Ambos campos son necesarios',
                error: true
            });
            return;
        }
        ...
        <div className='mt-20 md:mt-5 shadow-lg px-5 py-10 rounded-xl bg-white'>
            ...
            <form action="" onSubmit={handleSubmit}>
                <div className='my-5'>
                    ...
                    placeholder='Email de Registro' type="email" value={email} onChange={e => setEmail(e.target.value)} />
                </div>

                <div className='my-5'>
                    <label className='uppercase text-gray-600 block text-xl font-bold' htmlFor="">Password</label>
                    <input className='border w-full p-3 mt-3 bg-gray-50 rounded-xl' 
                    placeholder='Tu Contraseña' type="password" value={password} onChange={e => setPassword(e.target.value)} />
                </div>
